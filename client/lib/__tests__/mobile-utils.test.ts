...
import { detectMobile, getMobileFileAccept, getDownloadInstructions } from '../mobile-utils';
...
describe('Mobile Utils', () => {
  test('detectMobile should identify mobile devices', () => {
    // Test iOS
    mockNavigator('Mozilla/5.0 (iPhone; CPU iPhone OS 14_0 like Mac OS X) AppleWebKit/605.1.15');
  ...
  test('getMobileFileAccept should return appropriate accept attributes', () => {
    ...
    expect(getMobileFileAccept('image')).toBe('image/*,.jpg,.jpeg,.png,.gif,.webp,.bmp');
  });
