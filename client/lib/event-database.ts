...
export const eventDatabase = {
  ...
  photos: {
    async getByEventType(
      eventType: "violet_haldi" | "aral_roce",
    ): Promise<EventPhoto[]> {
      if (!isSupabaseConfigured()) {
        console.warn(
          "Supabase not configured. Event photos require Supabase connection.",
        );
        return [];
      }
    ...
    async create(
      photo: Omit<EventPhoto, "id" | "created_at" | "updated_at">,
    ...
    getDefaultMessage(messageKey: string): EventMessage {
      const defaultMessages: Record<string, EventMessage> = {
        ...
        post_wedding: {
          ...
          title:
            "Wedding is done — we'll be back soon with something exciting!",
